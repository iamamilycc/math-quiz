...
按「英语教学 + 中文母语学习者 + iPad/中文输入法」三类常见坑逐条实测（2026-09-09）：

| 坑 | 实测结果 |
|---|---|
| 撇号变体（iPad 智能标点 `'`→`’`） | ❌ 中招，已根治（见下文） |
| **中文输入法的全角标点** | ❌ **中招**：打「。」被判「句尾少了标点」，孩子眼里明明打了句号 → 已改成**不判错但提醒切英文输入法** |
| **英式/美式拼写** | ❌ **中招**：新概念是英式教材，词表收 `colour/favourite/grey/mum` 等 11 个词，孩子写美式会被判错 → 已改成**算对 + 说清两边口径** |
| **TTS 口音** | ❌ **中招**：音标是英式 `/ɑː/ /ɒ/`，朗读却是 `en-US` → 已改 `en-GB`（**语音识别仍用 en-US**，识别引擎对非母语者的美式模型支持最好，两者不能一起改） |
| 输入框自动大写/自动纠错 | ✅ 三个输入框都已 `autocapitalize/autocorrect/spellcheck` 全关 |
| fill 题缩写等价（`don't` vs `do not`） | ✅ 99 题全都收了两种形式 |
| 音标体系混用（英式/美式儿化 r） | ✅ 601 词全英式，0 处混用 |
| 例句超纲（用了后面课才学的词） | ⚪ 非问题：小学/初中例句用 `where/cup/tea` 完全合理，按课次卡例句用词反而不自然 |
| **同音词/形近词判分** | ❌ **中招**：写成 `here` 只给字母对比「差在这里：h[e]a[]r」，孩子会以为自己拼错了 —— 其实是**两个词记混了** → 已改成讲两个词的区别（90 组，双向对称，本册命中 31 个词） |
...
判分不是只比字符串。孩子写错的方式有三类，处理方式完全不同：

| 他写的 | 该怎么判 | 为什么 |
|---|---|---|
| `color`（答案 `colour`） | **算对** + 说清英式/美式 | 他没拼错。判错会让他以为 color 是错的，比不给分更糟 |
| `here`（答案 `hear`） | **判错** + 讲两个词的区别 | 不是拼错，是两个词记混了。给字母对比只会误导成「我拼错了」 |
| `heer`（答案 `hear`） | **判错** + 逐字母高亮 | 这才是真拼错，该看差在哪个字母 |

`CONFUSABLE` 表 90 组、**双向对称**（写反了也要认得，这条是测试逼出来的），
本册词表命中 31 个词（hear/here、son/sun、meet/meat、wear/where、flour/flower…）。
同音词和形近词的措辞分开：前者说「读音一模一样，只能靠拼写和意思分辨」，后者说「长得很像，特别容易混」。

两个站共用同一套判定（精读站生词卡也接上了），各自有真点击测试盯着。
...
| `TEST003` 模块没有同名测试 | **不适用**。测试按功能命名（tests_grammar / tests_resume / tests_ai…），不按模块名，15 支覆盖到位。 |
| `DOC002` 找不到教程档 | **本文件就是**。体检只认特定文件名。 |

## 自查清单（每次改完照着走）

1. `python3 build/build_words.py` —— 体检不过就不生成文件
2. 15 支测试全绿（见上面「一次改完要跑的全套」）
